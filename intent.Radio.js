...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.Radio.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.Radio.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent Radio, Syntax: true');
		state.isIntent = true;
